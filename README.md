...
- `DATABASE_URL`
- `DATABASE_AUTH_TOKEN`
- `SERPAPI_API_KEY` — 主要 SerpApi 金鑰（另可設定 `SERPAPI_API_KEY_POOL`）
- `RSS_FEED_URLS`
- `DISCORD_WEBHOOK_URL`
...
- `SERPAPI_API_KEY_POOL`
  - 以逗號分隔的額外 SerpApi 金鑰清單，用於輪替使用（請參閱 [SerpApi 用量說明](#serpapi-用量與-api-金鑰池繁)）
  - 範例：`SERPAPI_API_KEY_POOL=key_b,key_c`
  - 當所有金鑰達到用量上限（HTTP 403/429）時，服務將自動降級處理
- `OPENAI_API_KEY`
- `OPENAI_MODEL` 預設：`gpt-4o-mini`
- `GROQ_API_KEY`
- `BUSINESS_DEAL_THRESHOLD_GBP` 預設：`1000`
- `BUSINESS_DEAL_MIN_CONFIDENCE` 預設：`0.8`
- `NORMAL_FARES_CRON` 預設：`0 */6 * * *`
...
每條追蹤航線每次掃描約消耗 **3 次 SerpApi 呼叫**（每個滾動出發日期視窗各一次）。以預設每 6 小時執行一次的排程計算，單條航線每月約產生 **~360 次呼叫**。
...
|---|---|---|
| 1–2 條 | ~360–720 | 2 組 |
| 3–5 條 | ~1,080–1,800 | 2–3 組 |
| 6–10 條 | ~2,160–3,600 | 3 組以上 |

> **建議**：申請 2–3 個 SerpApi 免費帳號，將金鑰填入 `SERPAPI_API_KEY_POOL`。當某組金鑰達到用量上限（HTTP 403/429）時，金鑰池會自動切換至下一組。
...
*注意：在執行一般票價任務之前，`tracked_destinations` 資料表中通常需要至少一筆記錄。*
...
| `npm run init:database` | 從編譯腳本初始化資料庫物件 |
| `npm run seed:tracked-destinations` | 植入追蹤航線資料 |
...
- `DATABASE_URL`
- `DATABASE_AUTH_TOKEN`
- `SERPAPI_API_KEY` — 主 SerpApi 密钥（另可配置 `SERPAPI_API_KEY_POOL`）
- `RSS_FEED_URLS`
- `DISCORD_WEBHOOK_URL`
...
- `SERPAPI_API_KEY_POOL`
  - 以逗号分隔的额外 SerpApi 密钥列表，用于轮换使用（请参阅 [SerpApi 用量说明](#serpapi-用量与-api-密钥池简)）
  - 示例：`SERPAPI_API_KEY_POOL=key_b,key_c`
  - 当所有密钥达到用量上限（HTTP 403/429）时，服务将自动降级处理
- `OPENAI_API_KEY`
- `OPENAI_MODEL` 默认：`gpt-4o-mini`
- `GROQ_API_KEY`
- `BUSINESS_DEAL_THRESHOLD_GBP` 默认：`1000`
- `BUSINESS_DEAL_MIN_CONFIDENCE` 默认：`0.8`
- `NORMAL_FARES_CRON` 默认：`0 */6 * * *`
...
每条追踪航线每次扫描约消耗 **3 次 SerpApi 调用**（每个滚动出发日期窗口各一次）。以默认每 6 小时执行一次的定时任务计算，单条航线每月约产生 **~360 次调用**。
...
|---|---|---|
| 1–2 条 | ~360–720 | 2 组 |
| 3–5 条 | ~1,080–1,800 | 2–3 组 |
| 6–10 条 | ~2,160–3,600 | 3 组以上 |

> **建议**：注册 2–3 个 SerpApi 免费账号，将密钥填入 `SERPAPI_API_KEY_POOL`。当某组密钥达到用量上限（HTTP 403/429）时，密钥池会自动切换至下一组。
...
*注意：在运行普通票价任务之前，`tracked_destinations` 表中通常需要至少一条记录。*
...
| `npm run init:database` | 从编译脚本初始化数据库对象 |
| `npm run seed:tracked-destinations` | 填充追踪航线数据 |
