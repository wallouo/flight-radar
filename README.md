...
> ⚠️ Tests run against compiled files — always `npm run build` before `npm test`.
...
#### 🐛 Scan dates hardcoded to early June
...
#### 🐛 `deep_search=true` missing from SerpApi requests
...
> 💡 建議申請 2–3 個 SerpApi 免費帳號，並將所有金鑰填入 `SERPAPI_API_KEYS`。
...
> 💡 建议注册 2–3 个 SerpApi 免费账号，将所有密钥填入 `SERPAPI_API_KEYS`。
...
Made with ❤️ · [Report an Issue](https://github.com/wallouo/flight-radar/issues)
